...
function firstVersionDigits(value) {
...
function findSetupVersion(steps, action) {
...
function escapeForMarkdownCodeBlock(value) {
  return value.replace(/```/g, "`\\`\\`");
}

function collectSecretAndEnvPlaceholders(rawWorkflow) {
  const vars = new Set();
...
function generateRunScript(jobName, runCommands) {
...
function generateReadme(targetWorkflow, targetJob, runCommands, ignoredUses, warnings, placeholders) {
  const lines = [];
...
export async function reproGenerate(rawInput) {
