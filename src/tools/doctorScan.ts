...
export type DoctorScanInput = z.infer<typeof doctorScanInputSchema>;
...
function parseMakeTargets(makefileContent: string): Set<string> {
...
function hasPinnedImage(image: string): { pinned: boolean; reason: string } {
...
function parseDockerfileFromImages(content: string): string[] {
  const images: string[] = [];
...
function buildQuickstart(repoPath: string, workflows: string[]): DoctorReport["quickstart"] {
...
function compareMajorVersions(values: string[]): Set<string> {
  return new Set(values.map(extractMajor).filter((v) => v !== "unknown"));
}

export async function doctorScan(rawInput: unknown): Promise<DoctorReport> {
  ...
  if (input.focus.includes("ci")) {
    ...
    if (runCommands.length > 0) {
      ...
      if (await fileExists(packagePath)) {
        ...
        const pkg = JSON.parse(packageRaw) as { scripts?: Record<string, unknown> };
        scripts = pkg.scripts ?? {};
      }
      ...
      const missingEquivalents: string[] = [];
      ...
      for (const run of runCommands) {
        ...
        for (const line of run.command.split(/\r?\n/)) {
          ...
          if (/^npm\s+test(\s|$)/.test(trimmed) && !("test" in scripts)) {
